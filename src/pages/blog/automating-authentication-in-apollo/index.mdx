...
As mentioned above, this header will append the token onto every request that is sent to our GraphQL API. The double curly braces used by \{\{token\}\} tells Apollo to replace it with the environment variable named token.

That's it! At this point you should now be able to make authenticated requests to your GraphQL API from your Apollo Sandbox.
